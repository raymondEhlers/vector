...
def test_xy_xy():
    v1 = vector._backends.object_.VectorObject2D(
        azimuthal=vector._backends.object_.AzimuthalObjectXY(1, 2)
    )
...
def test_xy_rhophi():
    ...
    v2 = vector._backends.object_.VectorObject2D(
        azimuthal=vector._backends.object_.AzimuthalObjectRhoPhi(3, 4)
    )
...
def test_rhophi_xy():
    v1 = vector._backends.object_.VectorObject2D(
        azimuthal=vector._backends.object_.AzimuthalObjectRhoPhi(1, 2)
    )
    v2 = vector._backends.object_.VectorObject2D(
        azimuthal=vector._backends.object_.AzimuthalObjectXY(3, 4)
    )
